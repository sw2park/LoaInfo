...
import com.toy.LoaInfo.api.LoaApiConst;
...
public class APIController {
	...
	public String callContentsAPI(String _category, String _cateDetail) {
		StringBuilder response = new StringBuilder();
		try {
			// 카테고리 인코딩
			String category = URLEncoder.encode(_category, "UTF-8");
			...
			// URL API 설정
			URL url = new URL(LoaApiConst.LOA_API_URL+category+"/"+cateDetail);
			HttpURLConnection httpURLConnection = (HttpURLConnection) url.openConnection();
	        httpURLConnection.setRequestMethod("GET");
	        httpURLConnection.setRequestProperty("authorization", "Bearer " + LoaApiConst.LOA_API_KEY);
	        httpURLConnection.setRequestProperty("accept", "application/json");
